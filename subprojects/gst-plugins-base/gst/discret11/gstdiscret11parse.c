...
/**
 * SECTION:gstdiscret11parse
 * @short_description: Discret11 class for media data parsers
 *
 ...
 *
 * Use the GST_DISCRET11_CONFIG_MUTEX_LOCK and GST_DISCRET11_CONFIG_MUTEX_UNLOCK
 ...
 *   GstDiscret11 lock. This is typically necessary when the
 ...
 *   setting new property values), gst_discret11_invalidate_src_caps()
...
GST_DEBUG_CATEGORY_STATIC (discret11_parse_debug);
#define GST_CAT_DEFAULT discret11_parse_debug
...
#define INITIAL_PARSER_CONFIG \
  ((DEFAULT_USE_SINK_CAPS) ? GST_DISCRET11_PARSE_CONFIG_SINKCAPS : \
   GST_DISCRET11_PARSE_CONFIG_PROPERTIES)

#define gst_discret11_parse_parent_class parent_class
G_DEFINE_ABSTRACT_TYPE (GstDiscret11Parse, gst_discret11_parse,
    GST_TYPE_BASE_PARSE);

static void gst_discret11_parse_finalize (GObject * object);
static void gst_discret11_parse_set_property (GObject * object, guint prop_id,
    GValue const *value, GParamSpec * pspec);
static void gst_discret11_parse_get_property (GObject * object, guint prop_id,
    GValue * value, GParamSpec * pspec);
static gboolean gst_discret11_parse_start (GstBaseParse * parse);
static gboolean gst_discret11_parse_stop (GstBaseParse * parse);
static gboolean gst_discret11_parse_set_sink_caps (GstBaseParse * parse,
    GstCaps * caps);
static GstFlowReturn gst_discret11_parse_handle_frame (GstBaseParse * parse,
    GstBaseParseFrame * frame, gint * skipsize);
static gboolean gst_discret11_parse_convert (GstBaseParse * parse,
    GstFormat src_format, gint64 src_value, GstFormat dest_format,
    gint64 * dest_value);

static gboolean gst_discret11_parse_is_using_sink_caps (GstDiscret11Parse *
    discret11);
static gboolean gst_discret11_parse_is_gstformat_supported (GstDiscret11Parse *
    discret11, GstFormat format);

static void
gst_discret11_parse_class_init (GstDiscret11ParseClass * klass)
{
  GObjectClass *object_class;
  GstBaseParseClass *baseparse_class;

  GST_DEBUG_CATEGORY_INIT (discret11_parse_debug, "discret11parse", 0,
      "base discret11 parse class");

  object_class = G_OBJECT_CLASS (klass);
  baseparse_class = GST_BASE_PARSE_CLASS (klass);

  object_class->finalize = GST_DEBUG_FUNCPTR (gst_discret11_parse_finalize);
  object_class->set_property =
      GST_DEBUG_FUNCPTR (gst_discret11_parse_set_property);
  object_class->get_property =
      GST_DEBUG_FUNCPTR (gst_discret11_parse_get_property);

  baseparse_class->start = GST_DEBUG_FUNCPTR (gst_discret11_parse_start);
  baseparse_class->stop = GST_DEBUG_FUNCPTR (gst_discret11_parse_stop);
  baseparse_class->set_sink_caps =
      GST_DEBUG_FUNCPTR (gst_discret11_parse_set_sink_caps);
  baseparse_class->handle_frame =
      GST_DEBUG_FUNCPTR (gst_discret11_parse_handle_frame);
  baseparse_class->convert = GST_DEBUG_FUNCPTR (gst_discret11_parse_convert);

  /**
   * GstDiscret11Parse::use-sink-caps:
   *
  ...
  gst_type_mark_as_plugin_api (GST_TYPE_DISCRET11_PARSE, 0);
}

static void
gst_discret11_parse_init (GstDiscret11Parse * discret11)
{
  discret11->src_caps_set = FALSE;
  g_mutex_init (&(discret11->config_mutex));
}

static void
gst_discret11_parse_finalize (GObject * object)
{
  GstDiscret11Parse *discret11 = GST_DISCRET11_PARSE (object);

  g_mutex_clear (&(discret11->config_mutex));

  G_OBJECT_CLASS (parent_class)->finalize (object);
}

static void
gst_discret11_parse_set_property (GObject * object, guint prop_id,
    GValue const *value, GParamSpec * pspec)
{
  GstBaseParse *base_parse = GST_BASE_PARSE (object);
  GstDiscret11Parse *discret11 = GST_DISCRET11_PARSE (object);
  GstDiscret11ParseClass *klass = GST_DISCRET11_PARSE_GET_CLASS (object);
  ...
  switch (prop_id) {
    case PROP_USE_SINK_CAPS:
    {
      ...
      GstDiscret11ParseConfig new_config;
      ...
      cur_state = gst_discret11_parse_is_using_sink_caps (discret11);
      if (new_state == cur_state) {
        GST_DISCRET11_PARSE_CONFIG_MUTEX_UNLOCK (object);
        break;
      }

      GST_DEBUG_OBJECT (discret11, "switching to %s config",
          new_state ? "sink caps" : "properties");
      new_config =
          new_state ? GST_DISCRET11_PARSE_CONFIG_SINKCAPS :
          GST_DISCRET11_PARSE_CONFIG_PROPERTIES;

      if (!klass->set_current_config (discret11, new_config)) {
        GST_DISCRET11_PARSE_CONFIG_MUTEX_UNLOCK (object);
        GST_ELEMENT_ERROR (discret11, STREAM, FAILED,
            ("could not set new current config"),
            ("use-sink-caps property: %d", new_state));
        break;
      }
       ...
       * well. */
      if (klass->is_config_ready (discret11,
              GST_DISCRET11_PARSE_CONFIG_CURRENT)) {
        gsize frame_size = klass->get_config_frame_size (discret11,
            GST_DISCRET11_PARSE_CONFIG_CURRENT);
...
static void
gst_discret11_parse_get_property (GObject * object, guint prop_id,
    GValue * value, GParamSpec * pspec)
{
  GstDiscret11Parse *discret11 = GST_DISCRET11_PARSE (object);
  ...
  switch (prop_id) {
    case PROP_USE_SINK_CAPS:
      ...
      g_value_set_boolean (value,
          gst_discret11_parse_is_using_sink_caps (discret11));
      GST_DISCRET11_PARSE_CONFIG_MUTEX_UNLOCK (object);
      break;
...
static gboolean
gst_discret11_parse_start (GstBaseParse * parse)
{
  GstBaseParse *base_parse = GST_BASE_PARSE (parse);
  GstDiscret11Parse *discret11 = GST_DISCRET11_PARSE (parse);
  GstDiscret11ParseClass *klass = GST_DISCRET11_PARSE_GET_CLASS (parse);

  g_assert (klass->set_current_config);

  GST_DISCRET11_PARSE_CONFIG_MUTEX_LOCK (discret11);

  /* If the config is ready from the start, set the min frame size
   * (this will happen with the properties config) */
  if (klass->is_config_ready (discret11, GST_DISCRET11_PARSE_CONFIG_CURRENT)) {
    gsize frame_size = klass->get_config_frame_size (discret11,
        GST_DISCRET11_PARSE_CONFIG_CURRENT);
    gst_base_parse_set_min_frame_size (base_parse, frame_size);
  }

  GST_DISCRET11_PARSE_CONFIG_MUTEX_UNLOCK (discret11);

  return TRUE;
}

static gboolean
gst_discret11_parse_stop (GstBaseParse * parse)
{
  GstDiscret11Parse *discret11 = GST_DISCRET11_PARSE (parse);
...
static gboolean
gst_discret11_parse_set_sink_caps (GstBaseParse * parse, GstCaps * caps)
{
  gboolean ret = FALSE;
  GstDiscret11Parse *discret11 = GST_DISCRET11_PARSE (parse);
  GstDiscret11ParseClass *klass = GST_DISCRET11_PARSE_GET_CLASS (parse);
   ...
   * readies the config. */
  ret =
      klass->set_config_from_caps (discret11,
      GST_DISCRET11_PARSE_CONFIG_SINKCAPS, caps);
  if (!ret) {
    GST_ERROR_OBJECT (discret11, "could not get config from sink caps");
    goto done;
  }
  ...
  if (gst_discret11_parse_is_using_sink_caps (discret11)) {
    ...
    if (!klass->get_caps_from_config (discret11,
            GST_DISCRET11_PARSE_CONFIG_CURRENT, &new_src_caps)) {
      GST_ERROR_OBJECT (discret11,
          "could not get src caps from current config");
      goto done;
    }

    GST_DEBUG_OBJECT (discret11,
        "got new sink caps; updating src caps to %" GST_PTR_FORMAT,
        (gpointer) new_src_caps);

    frame_size =
        klass->get_config_frame_size (discret11,
        GST_DISCRET11_PARSE_CONFIG_CURRENT);
    gst_base_parse_set_min_frame_size (parse, frame_size);

    discret11->src_caps_set = TRUE;

    GST_DISCRET11_PARSE_CONFIG_MUTEX_UNLOCK (discret11);

    /* Push caps outside of the lock */
    gst_pad_push_event (GST_BASE_PARSE_SRC_PAD (discret11),
...
static GstBuffer *
gst_discret11_parse_align_buffer (GstDiscret11Parse * discret11,
    gsize alignment, GstBuffer * buffer, gsize out_size)
{
  ...
  if (((guintptr) map.data) & (alignment - 1)) {
    ...
    gst_buffer_copy_into (new_buffer, buffer,
        GST_BUFFER_COPY_METADATA, 0, out_size);
    GST_DEBUG_OBJECT (discret11,
...
gst_discret11_parse_handle_frame (GstBaseParse * parse,
    GstBaseParseFrame * frame, gint * skipsize)
{
  ...
  *skipsize = 0;
  ...
  if (G_UNLIKELY (!discret11->src_caps_set)) {
    GstCaps *new_src_caps;

    if (G_UNLIKELY (!klass->is_config_ready (discret11,
                GST_DISCRET11_PARSE_CONFIG_CURRENT))) {
  ...
  if (klass->get_max_frames_per_buffer) {
    guint max_num_out_frames = klass->get_max_frames_per_buffer (discret11,
        GST_DISCRET11_PARSE_CONFIG_CURRENT);
  ...
  frame->overhead =
      klass->get_overhead_size ? klass->get_overhead_size (discret11,
      GST_DISCRET11_PARSE_CONFIG_CURRENT) : 0;

  g_assert (out_size >= (guint) (frame->overhead));
  out_size -= frame->overhead;

  GST_LOG_OBJECT (discret11,
  ...
  klass->get_units_per_second (discret11, GST_FORMAT_BYTES,
      GST_DISCRET11_PARSE_CONFIG_CURRENT, &units_n, &units_d);
  ...
  if (klass->process) {
    ...
    if (!klass->process (discret11, GST_DISCRET11_PARSE_CONFIG_CURRENT,
  ...
  if (klass->get_alignment
      && (alignment =
          klass->get_alignment (discret11,
              GST_DISCRET11_PARSE_CONFIG_CURRENT)) != 1) {
    GstBuffer *aligned_buffer;

    aligned_buffer =
        gst_discret11_parse_align_buffer (discret11, alignment,
  ...
  if (G_UNLIKELY (new_caps_event != NULL)) {
    gst_pad_push_event (GST_BASE_PARSE_SRC_PAD (discret11), new_caps_event);
    new_caps_event = NULL;
  }
...
static gboolean
gst_discret11_parse_convert (GstBaseParse * parse, GstFormat src_format,
    gint64 src_value, GstFormat dest_format, gint64 * dest_value)
{
  ...
  if (G_UNLIKELY (src_format == dest_format)) {
    *dest_value = src_value;
      ...
      && gst_discret11_parse_is_gstformat_supported (discret11, src_format)
      && gst_discret11_parse_is_gstformat_supported (discret11, dest_format)) {
    ...
    if (src_format == GST_FORMAT_TIME) {
      ...
      klass->get_units_per_second (discret11, dest_format,
          GST_DISCRET11_PARSE_CONFIG_CURRENT, &units_n, &units_d);
      *dest_value = (units_n == 0
      ...
      klass->get_units_per_second (discret11, src_format,
          GST_DISCRET11_PARSE_CONFIG_CURRENT, &units_n, &units_d);
      *dest_value = (units_n == 0
          || units_d == 0) ? src_value : gst_util_uint64_scale (src_value,
...
static gboolean
gst_discret11_parse_is_using_sink_caps (GstDiscret11Parse * discret11)
{
  /* must be called with lock */
  GstDiscret11ParseClass *klass = GST_DISCRET11_PARSE_GET_CLASS (discret11);
  g_assert (klass->get_current_config);
  return klass->get_current_config (discret11) ==
      GST_DISCRET11_PARSE_CONFIG_SINKCAPS;
}

static gboolean
gst_discret11_parse_is_gstformat_supported (GstDiscret11Parse * discret11,
    GstFormat format)
{
  /* must be called with lock */
  GstDiscret11ParseClass *klass = GST_DISCRET11_PARSE_GET_CLASS (discret11);
  g_assert (klass->is_unit_format_supported);
  return klass->is_unit_format_supported (discret11, format);
}

/**
 * gst_discret11_parse_invalidate_src_caps:
 * @discret11: a #GstDiscret11Parse instance
 *
 ...
 * GST_DISCRET11_PARSE_CONFIG_MUTEX_LOCK() and GST_DISCRET11_PARSE_CONFIG_MUTEX_UNLOCK()
 * macros for this purpose.
 */
void
gst_discret11_parse_invalidate_src_caps (GstDiscret11Parse * discret11)
{
  /* must be called with lock */
  g_assert (discret11 != NULL);
  discret11->src_caps_set = FALSE;
}
