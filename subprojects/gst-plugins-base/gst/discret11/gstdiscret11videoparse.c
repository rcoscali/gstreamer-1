...
/**
 * SECTION:element-discret11videoparse
 * @title: discret11videoparse
 *
...
#include "gstdiscret11videoparse.h"
#include "unalignedvideo.h"

GST_DEBUG_CATEGORY_STATIC (discret11_video_parse_debug);
#define GST_CAT_DEFAULT discret11_video_parse_debug
...
#define GST_DISCRET11_VIDEO_PARSE_CAPS \
...
static GstStaticPadTemplate static_sink_template =
    ...
    GST_PAD_SINK,
    GST_PAD_ALWAYS,
    GST_STATIC_CAPS (GST_UNALIGNED_DISCRET11_VIDEO_CAPS "; "
        GST_DISCRET11_VIDEO_PARSE_CAPS)
    );
...
GST_STATIC_PAD_TEMPLATE ("src",
    GST_PAD_SRC,
    GST_PAD_ALWAYS,
    GST_STATIC_CAPS (GST_DISCRET11_VIDEO_PARSE_CAPS)
    );

#define gst_discret11_video_parse_parent_class parent_class
G_DEFINE_TYPE (GstDiscret11VideoParse, gst_discret11_video_parse,
    GST_TYPE_DISCRET11_PARSE);
GST_ELEMENT_REGISTER_DEFINE (discret11videoparse, "discret11videoparse",
    GST_RANK_NONE, GST_TYPE_DISCRET11_VIDEO_PARSE);

static void gst_discret11_video_parse_set_property (GObject * object,
    guint prop_id, GValue const *value, GParamSpec * pspec);
static void gst_discret11_video_parse_get_property (GObject * object,
    guint prop_id, GValue * value, GParamSpec * pspec);

static gboolean gst_discret11_video_parse_stop (GstBaseParse * parse);

static gboolean gst_discret11_video_parse_set_current_config (GstDiscret11Parse
    * discret11_parse, GstDiscret11ParseConfig config);
static GstDiscret11ParseConfig
gst_discret11_video_parse_get_current_config (GstDiscret11Parse *
    discret11_parse);
static gboolean
gst_discret11_video_parse_set_config_from_caps (GstDiscret11Parse *
    discret11_parse, GstDiscret11ParseConfig config, GstCaps * caps);
static gboolean
gst_discret11_video_parse_get_caps_from_config (GstDiscret11Parse *
    discret11_parse, GstDiscret11ParseConfig config, GstCaps ** caps);
static gsize gst_discret11_video_parse_get_config_frame_size (GstDiscret11Parse
    * discret11_parse, GstDiscret11ParseConfig config);
static guint
gst_discret11_video_parse_get_max_frames_per_buffer (GstDiscret11Parse *
    discret11_parse, GstDiscret11ParseConfig config);
static gboolean gst_discret11_video_parse_is_config_ready (GstDiscret11Parse *
    discret11_parse, GstDiscret11ParseConfig config);
static gboolean gst_discret11_video_parse_process (GstDiscret11Parse *
...
static gboolean
gst_discret11_video_parse_is_unit_format_supported (GstDiscret11Parse *
    discret11_parse, GstFormat format);
static void gst_discret11_video_parse_get_units_per_second (GstDiscret11Parse *
    discret11_parse, GstFormat format, GstDiscret11ParseConfig config,
    gsize * units_per_sec_n, gsize * units_per_sec_d);

static gint gst_discret11_video_parse_get_overhead_size (GstDiscret11Parse *
    discret11_parse, GstDiscret11ParseConfig config);

static gboolean gst_discret11_video_parse_is_using_sink_caps (GstDiscret11Parse
    * discret11_video_parse);
static GstDiscret11VideoParseConfig
    * gst_discret11_video_parse_get_config_ptr (GstDiscret11VideoParse *
    discret11_video_parse, GstDiscret11ParseConfig config);

static void gst_discret11_video_parse_init_config (GstDiscret11VideoParseConfig
    * config);
static void gst_discret11_video_parse_update_info (GstDiscret11VideoParseConfig
    * config);

static void
gst_discret11_video_parse_class_init (GstDiscret11VideoParseClass * klass)
{
  ...
  GST_DEBUG_CATEGORY_INIT (discret11_video_parse_debug, "discret11videoparse",
      0, "discret11videoparse element");
  ...
  object_class->set_property =
      GST_DEBUG_FUNCPTR (gst_discret11_video_parse_set_property);
  object_class->get_property =
      GST_DEBUG_FUNCPTR (gst_discret11_video_parse_get_property);

  baseparse_class->stop =
      /* (gboolean (*)(GstBaseParse *)) */
      GST_DEBUG_FUNCPTR (gst_discret11_video_parse_stop);

  discret11parse_class->set_current_config =    /*(gboolean (*)(GstDiscret11Parse *, GstDiscret11ParseConfig)) */
      GST_DEBUG_FUNCPTR (gst_discret11_video_parse_set_current_config);
  discret11parse_class->get_current_config =    /*(GstDiscret11ParseConfig (*)(GstDiscret11Parse *)) */
      GST_DEBUG_FUNCPTR (gst_discret11_video_parse_get_current_config);
  discret11parse_class->set_config_from_caps =  /*(gboolean (*)(GstDiscret11Parse *, GstDiscret11ParseConfig,  GstCaps *)) */
      GST_DEBUG_FUNCPTR (gst_discret11_video_parse_set_config_from_caps);
  discret11parse_class->get_caps_from_config =  /*(gboolean (*)(GstDiscret11Parse *, GstDiscret11ParseConfig,  GstCaps **)) */
      GST_DEBUG_FUNCPTR (gst_discret11_video_parse_get_caps_from_config);
  discret11parse_class->get_config_frame_size = /*(gsize (*)(GstDiscret11Parse *, GstDiscret11ParseConfig)) */
      GST_DEBUG_FUNCPTR (gst_discret11_video_parse_get_config_frame_size);
  discret11parse_class->get_max_frames_per_buffer =     /*(guint (*)(GstDiscret11Parse *, GstDiscret11ParseConfig)) */
      GST_DEBUG_FUNCPTR (gst_discret11_video_parse_get_max_frames_per_buffer);
  discret11parse_class->is_config_ready =
      GST_DEBUG_FUNCPTR (gst_discret11_video_parse_is_config_ready);
  discret11parse_class->process =
      GST_DEBUG_FUNCPTR (gst_discret11_video_parse_process);
  discret11parse_class->is_unit_format_supported =
      GST_DEBUG_FUNCPTR (gst_discret11_video_parse_is_unit_format_supported);
  discret11parse_class->get_units_per_second =
      GST_DEBUG_FUNCPTR (gst_discret11_video_parse_get_units_per_second);
  discret11parse_class->get_overhead_size =
      GST_DEBUG_FUNCPTR (gst_discret11_video_parse_get_overhead_size);
...
static void
gst_discret11_video_parse_init (GstDiscret11VideoParse * discret11_video_parse)
{
  gst_discret11_video_parse_init_config (&
      (discret11_video_parse->properties_config));
  gst_discret11_video_parse_init_config (&
      (discret11_video_parse->sink_caps_config));

  /* As required by GstDiscret11VideoParse, ensure that the current configuration
   * is initially set to be the properties config */
  discret11_video_parse->current_config =
      &(discret11_video_parse->properties_config);

  /* Properties config must be valid from the start, so set its ready value
   * to TRUE, and make sure its bpf value is valid. */
  discret11_video_parse->properties_config.ready = TRUE;
  discret11_video_parse->properties_config.top_field_first =
      DEFAULT_TOP_FIELD_FIRST;
  discret11_video_parse->properties_config.frame_size = DEFAULT_FRAME_STRIDE;
}

static void
gst_discret11_video_parse_set_property (GObject * object, guint prop_id,
    GValue const *value, GParamSpec * pspec)
{
  GstBaseParse *base_parse = GST_BASE_PARSE (object);
  GstDiscret11Parse *discret11_parse = GST_DISCRET11_PARSE (object);
  GstDiscret11VideoParse *discret11_video_parse =
      GST_DISCRET11_VIDEO_PARSE (object);
  GstDiscret11VideoParseConfig *props_cfg =
      &(discret11_video_parse->properties_config);
   ...
   *   it is set to the result of gst_discret11_video_parse_get_config_frame_size()
   ...
   *   fields instead (gst_discret11_video_parse_update_info() then copies the values
   *   from these fields into the video info); see the documentation inside
   *   gst_discret11_video_parse_update_info() for the reason why
   */
  ...
  switch (prop_id) {
    case PROP_WIDTH:
    {
      ...
      if (new_width != props_cfg->width) {
        ...
        gst_discret11_video_parse_update_info (props_cfg);

        if (!gst_discret11_video_parse_is_using_sink_caps (discret11_parse)) {
          gst_discret11_parse_invalidate_src_caps (discret11_parse);
          gst_base_parse_set_min_frame_size (base_parse,
              gst_discret11_video_parse_get_config_frame_size (discret11_parse,
    ...
    case PROP_PIXEL_ASPECT_RATIO:
    {
      ...
      GST_DEBUG_OBJECT (discret11_video_parse,
          "setting pixel aspect ratio to %u/%u",
    ...
    case PROP_FRAMERATE:
    {
      ...
      props_cfg->framerate_d = GST_VIDEO_INFO_FPS_D (&(props_cfg->info)) =
          gst_value_get_fraction_denominator (value);
      GST_DEBUG_OBJECT (discret11_video_parse, "setting framerate to %u/%u",
    ...
    case PROP_PLANE_STRIDES:
    {
      ...
      if (gst_value_array_get_size (value) < n_planes) {
        GST_ELEMENT_ERROR (discret11_video_parse, LIBRARY, SETTINGS,
      ...
      for (i = 0; i < n_planes; ++i) {
        ...
        GST_DEBUG_OBJECT (discret11_video_parse, "plane #%u stride: %d", i,
    ...
    case PROP_PLANE_OFFSETS:
    {
      ...
      for (i = 0; i < n_planes; ++i) {
        ...
        GST_DEBUG_OBJECT (discret11_video_parse,
            "plane #%u offset: %" G_GSIZE_FORMAT, i,
            props_cfg->plane_offsets[i]);
      }
...
static void
gst_discret11_video_parse_get_property (GObject * object, guint prop_id,
  ...
  switch (prop_id) {
    ...
    case PROP_FRAME_SIZE:
      ...
      g_value_set_uint (value,
          discret11_video_parse->properties_config.frame_size);
      GST_DISCRET11_PARSE_CONFIG_MUTEX_UNLOCK (object);
      break;
...
static gboolean
gst_discret11_video_parse_stop (GstBaseParse * parse)
{
  GstDiscret11VideoParse *discret11_video_parse =
      GST_DISCRET11_VIDEO_PARSE (parse);
  ...
  discret11_video_parse->sink_caps_config.ready = FALSE;

  return GST_BASE_PARSE_CLASS (parent_class)->stop (parse);
}

static gboolean
gst_discret11_video_parse_set_current_config (GstDiscret11Parse *
    discret11_parse, GstDiscret11ParseConfig config)
{
  GstDiscret11VideoParse *discret11_video_parse =
      GST_DISCRET11_VIDEO_PARSE (discret11_parse);
...
gst_discret11_video_parse_get_current_config (GstDiscret11Parse *
    discret11_parse)
{
  return gst_discret11_video_parse_is_using_sink_caps (discret11_parse) ?
    ...
    discret11_parse, GstDiscret11ParseConfig config, GstCaps * caps)
{
  ...
  GstDiscret11VideoParse *discret11_video_parse =
      GST_DISCRET11_VIDEO_PARSE (discret11_parse);
  GstDiscret11VideoParseConfig *config_ptr =
      gst_discret11_video_parse_get_config_ptr (discret11_video_parse, config);
...
static gsize
gst_discret11_video_parse_get_config_frame_size (GstDiscret11Parse *
    discret11_parse, GstDiscret11ParseConfig config)
{
  GstDiscret11VideoParse *discret11_video_parse =
      GST_DISCRET11_VIDEO_PARSE (discret11_parse);
...
static guint
gst_discret11_video_parse_get_max_frames_per_buffer (G_GNUC_UNUSED
    GstDiscret11Parse * discret11_parse,
    G_GNUC_UNUSED GstDiscret11ParseConfig config)
{
...
static gboolean
gst_discret11_video_parse_is_config_ready (GstDiscret11Parse * discret11_parse,
    GstDiscret11ParseConfig config)
{
  GstDiscret11VideoParse *discret11_video_parse =
      GST_DISCRET11_VIDEO_PARSE (discret11_parse);
  return gst_discret11_video_parse_get_config_ptr (discret11_video_parse,
      config)->ready;
}

static gboolean
gst_discret11_video_parse_process (GstDiscret11Parse * discret11_parse,
    GstDiscret11ParseConfig config, GstBuffer * in_data,
    ...
    G_GNUC_UNUSED gsize num_valid_in_bytes, GstBuffer ** processed_data)
{
  ...
  if (!gst_buffer_map (in_data, &map_info, GST_MAP_READ)) {
    GST_WARNING_OBJECT (discret11_video_parse, "Failed to map input data");
    return FALSE;
  }
...
static gboolean
gst_discret11_video_parse_is_unit_format_supported (G_GNUC_UNUSED
    GstDiscret11Parse * discret11_parse, GstFormat format)
{
  switch (format) {
...
static void
gst_discret11_video_parse_get_units_per_second (GstDiscret11Parse *
    discret11_parse, GstFormat format, GstDiscret11ParseConfig config,
    gsize * units_per_sec_n, gsize * units_per_sec_d)
{
  GstDiscret11VideoParse *discret11_video_parse =
...
static gint
gst_discret11_video_parse_get_overhead_size (GstDiscret11Parse *
    discret11_parse, GstDiscret11ParseConfig config)
{
  GstDiscret11VideoParse *discret11_video_parse =
      GST_DISCRET11_VIDEO_PARSE (discret11_parse);
  ...
  GST_LOG_OBJECT (discret11_video_parse,
...
static gboolean
gst_discret11_video_parse_is_using_sink_caps (GstDiscret11Parse *
    discret11_parse)
{
  GstDiscret11VideoParse *discret11_video_parse =
      GST_DISCRET11_VIDEO_PARSE (discret11_parse);
  return discret11_video_parse->current_config ==
      &(discret11_video_parse->sink_caps_config);
}
...
gst_discret11_video_parse_get_config_ptr (GstDiscret11VideoParse *
    discret11_video_parse, GstDiscret11ParseConfig config)
{
  g_assert (discret11_video_parse->current_config != NULL);

  switch (config) {
    case GST_DISCRET11_PARSE_CONFIG_PROPERTIES:
      return &(discret11_video_parse->properties_config);

    case GST_DISCRET11_PARSE_CONFIG_SINKCAPS:
      return &(discret11_video_parse->sink_caps_config);
...
static void
gst_discret11_video_parse_init_config (GstDiscret11VideoParseConfig * config)
{
  int i;
...
static void
gst_discret11_video_parse_update_info (GstDiscret11VideoParseConfig * config)
{
  guint i;
  guint n_planes;
  guint last_plane;
